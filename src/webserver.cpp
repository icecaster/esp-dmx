...
static String buildPage() {
    ...
    p += F("<!DOCTYPE html><html><head>"
           ...
           "<style>"
           ":root{--g:#00ff00;--gd:#00b300;--bg:#0a0a0a;--s:#111;--b:#1a1a1a;--t:#e0e0e0;--bd:#2a2a2a}"
           "*{box-sizing:border-box}"
           "body{font-family:'Courier New',monospace;background:var(--bg);color:var(--t);"
           "max-width:520px;margin:20px auto;padding:0 14px}"
           "h1{margin:0 0 14px;color:var(--g);letter-spacing:2px;text-transform:uppercase;"
           "font-size:20px;border-bottom:1px solid var(--g);padding-bottom:8px}"
           "fieldset{margin:10px 0;padding:10px 14px;border:1px solid var(--bd);"
           "background:var(--b);border-radius:3px}"
           "legend{color:var(--g);font-weight:bold;padding:0 6px;letter-spacing:1px;"
           "text-transform:uppercase;font-size:12px}"
           "label{display:block;margin:7px 0 2px;font-size:13px;color:#aaa}"
           "input[type=text],input[type=number]{"
           "width:100%;padding:6px 8px;background:var(--s);color:var(--g);"
           "border:1px solid #333;border-radius:2px;font-family:inherit;font-size:14px}"
           "input[type=text]:focus,input[type=number]:focus{"
           "outline:none;border-color:var(--g);box-shadow:0 0 0 1px var(--g)}"
           "input[type=checkbox],input[type=radio]{accent-color:var(--g);margin-right:6px}"
           "label:has(input[type=checkbox]),label:has(input[type=radio]){"
           "color:var(--t);cursor:pointer}"
           "input[type=submit]{background:transparent;color:var(--g);border:1px solid var(--g);"
           "padding:9px 24px;border-radius:2px;cursor:pointer;font-family:inherit;"
           "font-size:14px;margin-top:10px;letter-spacing:1px;text-transform:uppercase}"
           "input[type=submit]:hover{background:var(--g);color:#000}"
           "#status{background:var(--s);border:1px solid var(--bd);border-left:3px solid var(--g);"
           "padding:8px 12px;border-radius:2px;font-size:13px;margin-bottom:12px;color:#aaa}"
